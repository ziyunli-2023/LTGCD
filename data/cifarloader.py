...
def target_transfer(labeled_list):
    max_id = max(labeled_list)+1
    label_sort = sorted(labeled_list)
    map = -1*np.ones(max_id)
    for i in range(len(label_sort)):
        map[label_sort[i]] = i
    return map

def CIFAR10Loader_list(label_list, unlabel_list, root, batch_size, split='train', num_workers=0, labeled = True, aug=None,  shuffle=True ):
    if aug == None:
        transform = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),
    ...
    if labeled:
        map = target_transfer(label_list)
    else:
        map = target_transfer(unlabel_list)

    dataset = CIFAR10(root=root, split=split, labeled=labeled, transform=transform, target_transform=map, labeled_list=label_list, unlabeled_list=unlabel_list)
    loader = data.DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, num_workers=0)
    return loader, dataset



def CIFAR10Loader_224(label_list, unlabel_list, root, batch_size, split='train', num_workers=0, labeled = True, aug=None,  shuffle=True ):
    if aug == None:
        transform = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),
    ...
    dataset = CIFAR10(root=root, split=split, labeled=labeled, transform=transform, labeled_list=label_list, unlabeled_list=unlabel_list)
    loader = data.DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, num_workers=0)
    return loader, dataset
...
def CIFAR100Loader_list(label_list, unlabel_list, root, batch_size, split='train', num_workers=0, labeled = True, aug=None,  shuffle=True ):
    if aug == None:
        transform = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.507, 0.487, 0.441), (0.267, 0.256, 0.276)),
    ...
    dataset = CIFAR100(root=root, split=split, labeled=labeled, transform=transform, target_transform=map, labeled_list=label_list, unlabeled_list=unlabel_list)
    loader = data.DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, num_workers=0)
    return loader, dataset

def CIFAR100Loader_list_blank(label_list, unlabel_list, root, batch_size, split='train', num_workers=0, labeled = True, aug=None,  shuffle=True ):
    if aug == None:
        transform = transforms.Compose([
            transforms.ToTensor(),
            #transforms.Normalize((0.507, 0.487, 0.441), (0.267, 0.256, 0.276)),
...
def CIFAR100Loader_224(root, batch_size, split='train', num_workers=2, labeled=True, shuffle=True, aug=None, mode='80_10'):
    if aug == None:
        transform = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.507, 0.487, 0.441), (0.267, 0.256, 0.276)),
